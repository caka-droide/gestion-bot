...
from views import TicketButton, CloseButton, TicketRequestView, ReglementView, RoleMenuView, AvisTicketView
...
class GestionBot(commands.Bot):
    ...
    async def on_message(self, message):
        # ⚠️ Ne PAS supprimer / ne PAS ajouter `await self.process_commands(message)`
        # ici. Par défaut, commands.Bot définit un on_message qui appelle
        # process_commands automatiquement. Le Cog Activity (activity.py) a
        # SON PROPRE listener on_message qui gère la pause de salon + l'XP et
        # appelle lui-même process_commands au bon moment. Comme un listener
        # de Cog s'ajoute à ce on_message (il ne le remplace pas), garder le
        # comportement par défaut ici ferait tourner process_commands DEUX
        # FOIS par message (une fois ici, une fois dans Activity), donc
        # chaque commande (+rename, +staff, etc.) s'exécuterait deux fois.
        # On neutralise donc ce on_message pour laisser Activity être la
        # seule source d'appel à process_commands.
        pass
