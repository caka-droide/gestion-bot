...
SALON_DEMANDE_TICKET_ID = 1521181952697172161  # salon où les demandes de tickets arrivent pour validation par le staff
...
def est_staff(membre: discord.Member) -> bool:
    """Vérifie qu'un membre est staff/admin staff ou propriétaire (utilisable hors commandes, ex: boutons)."""
...
# ── Système de Tickets (Ouverture avec raison + validation staff) ───────────
async def creer_salon_ticket(guild: discord.Guild, membre: discord.Member, raison: str = None):
    ...
    await envoyer_log(guild, "🎫 Ticket Ouvert", f"Ticket créé par {membre.mention} ({ticket_channel.mention})", 0x00FF00, membre)
...
class TicketRequestView(discord.ui.View):
    ...
    @discord.ui.button(label="Accepter", style=discord.ButtonStyle.success, custom_id="accepter_demande_ticket_btn", emoji="✅")
    async def accepter(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not membre:
            await interaction.response.send_message("❌ Ce membre a quitté le serveur, impossible de créer le ticket.", ephemeral=True)
            return
    ...
    @discord.ui.button(label="Refuser", style=discord.ButtonStyle.danger, custom_id="refuser_demande_ticket_btn", emoji="✖️")
...
class TicketReasonModal(discord.ui.Modal, title="Ouvrir un ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        salon_demande = guild.get_channel(SALON_DEMANDE_TICKET_ID)
        if not salon_demande:
            await interaction.response.send_message("❌ Le salon de demandes de tickets est introuvable, contacte un admin.", ephemeral=True)
            return

        embed = discord.Embed(title="🎫 Nouvelle demande de ticket", color=0xF1C40F, timestamp=datetime.now())
        ...
        await interaction.response.send_message("✅ Ta demande a été envoyée au staff, tu seras notifié dès qu'elle sera traitée !", ephemeral=True)
...
async def setup_ticket_cmd(interaction: discord.Interaction):
    embed = discord.Embed(title="📞 Support et Tickets", description="Clique sur le bouton ci-dessous pour ouvrir un ticket. On te demandera la raison de ta demande, puis le staff devra la valider.", color=0x2b2d31)
