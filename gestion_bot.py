...
async def setup_hook():
    ...
    tickets_attente_data = await load("tickets_attente")
...
# ── Vérifie si un membre est staff (pour les interactions par bouton) ───────
def membre_est_staff(membre: discord.Member) -> bool:
    if membre.id == membre.guild.owner_id:
        return True
    roles_ids = [r.id for r in membre.roles]
    return ROLE_STAFF_ID in roles_ids or ROLE_ADMIN_STAFF_ID in roles_ids

# ── Bouton "Prendre en charge" (dans le salon d'attente) ────────────────────
class PriseEnChargeView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Prendre en charge", style=discord.ButtonStyle.success, custom_id="prendre_en_charge_btn", emoji="✅")
    async def bouton_prendre_en_charge(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not membre_est_staff(interaction.user):
            await interaction.response.send_message("❌ Seul le staff peut prendre en charge un ticket.", ephemeral=True)
            return

        message_id = str(interaction.message.id)
        data_ticket = tickets_attente_data.get(message_id)
        if not data_ticket:
            await interaction.response.send_message("❌ Ce ticket a déjà été pris en charge ou n'existe plus.", ephemeral=True)
            return

        await interaction.response.defer()

        guild = interaction.guild
        auteur = guild.get_member(data_ticket["user_id"])
        if auteur is None:
            try:
                auteur = await bot.fetch_user(data_ticket["user_id"])
            except discord.NotFound:
                await interaction.followup.send("❌ Impossible de retrouver l'auteur de ce ticket.", ephemeral=True)
                return
        raison = data_ticket["raison"]

        nom_salon = f"ticket-{auteur.name.lower()}"
        if discord.utils.get(guild.text_channels, name=nom_salon):
            await interaction.followup.send("❌ Un salon de ticket existe déjà pour cet utilisateur.", ephemeral=True)
            return
        ...
        if isinstance(auteur, discord.Member):
            overwrites[auteur] = discord.PermissionOverwrite(read_messages=True, send_messages=True)

        try:
            ticket_channel = await guild.create_text_channel(name=nom_salon, category=category, overwrites=overwrites)
        except discord.Forbidden:
            await interaction.followup.send("❌ Je n'ai pas la permission de créer le salon de ticket.", ephemeral=True)
            return
        ...
        embed_ticket.add_field(name="👤 Ouvert par", value=f"{auteur.mention} (`{auteur.id}`)", inline=False)
        embed_ticket.add_field(name="🛡️ Staff", value=f"{interaction.user.mention} (`{interaction.user.id}`)", inline=False)
        embed_ticket.add_field(name="📝 Raison", value=raison, inline=False)

        await ticket_channel.send(content=f"{auteur.mention} {interaction.user.mention}", embed=embed_ticket, view=CloseButton())

        # Met à jour le message dans le salon d'attente pour indiquer que le ticket est pris
        try:
            embed_attente = interaction.message.embeds[0]
            embed_attente.color = 0x2ECC71
            embed_attente.set_footer(text=f"✅ Pris en charge par {interaction.user}")
            await interaction.message.edit(embed=embed_attente, view=None)
        except Exception as e:
            print(f"Erreur mise à jour message d'attente : {e}")

        tickets_attente_data.pop(message_id, None)
        await save("tickets_attente", tickets_attente_data)

        await envoyer_log(guild, "🎫 Ticket Pris en Charge", f"Ticket de {auteur.mention} pris en charge par {interaction.user.mention} ({ticket_channel.mention})", 0x2ECC71, interaction.user)
        await interaction.followup.send(f"✅ Ticket créé : {ticket_channel.mention}", ephemeral=True)

# ── Formulaire de création de ticket (raison) ────────────────────────────────
class TicketModal(discord.ui.Modal, title="Ouvrir un ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        guild = interaction.guild
        salon_attente = guild.get_channel(SALON_ATTENTE_ID)
        if not salon_attente:
            await interaction.response.send_message("❌ Le salon d'attente n'est pas configuré correctement (`SALON_ATTENTE_ID`).", ephemeral=True)
            return

        embed = discord.Embed(title="🎫 Nouveau Ticket", color=0x2b2d31)
        ...
        embed.set_footer(text="En attente de prise en charge")

        message_ping = f"<@&{ROLE_STAFF_ID}> <@&{ROLE_ADMIN_STAFF_ID}>"
        message_attente = await salon_attente.send(content=message_ping, embed=embed, view=PriseEnChargeView())

        tickets_attente_data[str(message_attente.id)] = {
        ...
        await interaction.response.send_message("✅ Ta demande a bien été envoyée ! Un membre du staff va la prendre en charge sous peu.", ephemeral=True)
        await envoyer_log(guild, "🎫 Ticket en Attente", f"{interaction.user.mention} a ouvert une demande de ticket.", 0xF1C40F, interaction.user)
...
class TicketButton(discord.ui.View):
    ...
    async def bouton_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if any(d["user_id"] == interaction.user.id for d in tickets_attente_data.values()):
            await interaction.response.send_message("❌ Tu as déjà une demande de ticket en attente de prise en charge.", ephemeral=True)
            return
...
async def on_ready():
    ...
    bot.add_view(PriseEnChargeView())
...
async def help(ctx):
    ...
    embed.add_field(name="👑 Propriétaire", value=(
        "`/gw` : Lancer un giveaway.\n"
        "`/pause` / `/slowmode` : Gérer le flux des salons.\n"
        "`/setup_ticket` / `/setup_reglement` / `/setup_roles` : Configurer le serveur.\n"
        "`/classement` : Voir le classement XP complet."
    ), inline=False)
...
@app_commands.checks.has_permissions(manage_messages=True)
async def clear_cmd(interaction: discord.Interaction, montant: int = 100):
...
@bot.tree.command(name="classement", description="Top 10 (Propriétaire uniquement)")
@is_server_owner()
