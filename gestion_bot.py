...
avis_data = {}  # { "guild_id": [ {"theme":, "note":, "texte":, "image_url":, "auteur_id":, "timestamp":}, ... ] }
...
# ── Système d'Avis (thème libre + note par étoiles + commentaire + preuve) ───
@bot.tree.command(name="avis", description="Laisser un avis (thème + note + commentaire, photo optionnelle en preuve)")
@app_commands.describe(
    theme="Le thème de ton avis (ex : achat de tel service, staff, ticket...)",
    note="Ta note de 1 à 5 étoiles",
    texte="Ton commentaire / avis",
    image="Image en preuve, facultatif (ex : preuve d'achat / de réception)"
)
...
async def avis_cmd(interaction: discord.Interaction, theme: str, note: app_commands.Choice[int], texte: str, image: discord.Attachment = None):
    if image and not (image.content_type or "").startswith("image/"):
        await interaction.response.send_message("❌ Le fichier fourni en preuve doit être une image.", ephemeral=True)
        return
    ...
    embed.set_footer(text=f"Avis laissé par {interaction.user.display_name}", icon_url=interaction.user.display_avatar.url)
    embed.timestamp = datetime.now()
    if image:
        embed.set_image(url=image.url)

    # L'avis est posté publiquement dans le salon où la commande est tapée (ex : #avis).
    ...
    await envoyer_log(
        interaction.guild, "📝 Nouvel Avis",
        f"Thème : {theme}\nPosté dans : {interaction.channel.mention}\nNote : {etoiles}\nPar : {interaction.user.mention}",
        0xF1C40F, interaction.user
    )

@bot.tree.command(name="avis_stats", description="Voir la moyenne des avis pour un thème (ou tout le serveur)")
@app_commands.describe(theme="Le thème à inspecter (optionnel, sinon tous confondus)")
async def avis_stats_cmd(interaction: discord.Interaction, theme: str = None):
    ...
    if theme:
        ...
        avis_liste = [a for a in tous_avis if a["theme"].strip().lower() == theme_norm]
        titre = f"📊 Avis pour « {theme.strip()} »"
    else:
        avis_liste = tous_avis
        titre = "📊 Avis du serveur (tous thèmes confondus)"

    if not avis_liste:
        await interaction.response.send_message("❌ Aucun avis trouvé pour le moment.", ephemeral=True)
        return

    moyenne = sum(a["note"] for a in avis_liste) / len(avis_liste)
    arrondi = round(moyenne)
    etoiles_moy = "⭐" * arrondi + "☆" * (5 - arrondi)

    embed = discord.Embed(title=titre, color=0xF1C40F)
    embed.add_field(name="Nombre d'avis", value=str(len(avis_liste)), inline=True)
    embed.add_field(name="Moyenne", value=f"{etoiles_moy} ({moyenne:.1f}/5)", inline=True)

    if not theme:
        # Petit récap par thème pour repérer les plus notés (regroupés sans tenir compte de la casse)
        par_theme = {}
        for a in tous_avis:
            cle = a["theme"].strip().lower()
            par_theme.setdefault(cle, {"nom": a["theme"].strip(), "notes": []})
            par_theme[cle]["notes"].append(a["note"])

        lignes = []
        for data in sorted(par_theme.values(), key=lambda x: len(x["notes"]), reverse=True)[:10]:
            moy = sum(data["notes"]) / len(data["notes"])
            lignes.append(f"**{data['nom']}** — {moy:.1f}/5 ({len(data['notes'])} avis)")
        if lignes:
            embed.add_field(name="Détail par thème (top 10)", value="\n".join(lignes), inline=False)
