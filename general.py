...
class General(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        embed.add_field(name="🛡️ Staff", value=(
            ...
            "`+close` : Demander la fermeture du ticket en cours (ping staff/admin-staff).\n"
    ...
    async def avis_staff_cmd(self, interaction: discord.Interaction, staff: discord.Member):
        gid = str(interaction.guild.id)
        avis_du_staff = [
            a for a in self.store.avis_tickets.get(gid, []) if a["staff_id"] == staff.id
        ]
        if not avis_du_staff:
            await interaction.response.send_message(
                f"❌ Aucun avis de ticket trouvé pour {staff.mention}.", ephemeral=True
            )
            return
        ...
        embed.add_field(
            name="Moyenne", value=f"{'⭐' * arrondi}{'☆' * (5 - arrondi)} ({moyenne:.2f}/5)", inline=False
        )
