...
avis_data = {}  # { "guild_id": { "channel_id": [ {"auteur_id":, "note":, "texte":, "timestamp":}, ... ] } }
...
# ── Système d'Avis (note par étoiles + commentaire) ───────────────────────────
@bot.tree.command(name="avis", description="Laisser un avis (note + commentaire) sur un sujet/salon du serveur")
@app_commands.describe(
    salon="Le salon/sujet concerné par ton avis (sert juste à préciser de quoi tu parles)",
...
async def avis_cmd(interaction: discord.Interaction, salon: discord.TextChannel, note: app_commands.Choice[int], texte: str):
    etoiles = "⭐" * note.value + "☆" * (5 - note.value)

    embed = discord.Embed(title="📝 Nouvel avis", color=0xF1C40F)
    embed.add_field(name="Concerne", value=salon.mention, inline=True)
    ...
    # Le paramètre "salon" ne sert qu'à indiquer le sujet de l'avis (ex : un bot précis),
    # il ne redirige pas le message ailleurs.
    await interaction.response.send_message(embed=embed)

    gid = str(interaction.guild.id)
    cid = str(salon.id)
    avis_data.setdefault(gid, {}).setdefault(cid, []).append({
    ...
    await envoyer_log(
        ...
        f"Concerne : {salon.mention}\nPosté dans : {interaction.channel.mention}\nNote : {etoiles}\nPar : {interaction.user.mention}",
        0xF1C40F, interaction.user
    )

@bot.tree.command(name="avis_stats", description="Voir la moyenne des avis sur un sujet/salon (ou tout le serveur)")
@app_commands.describe(salon="Le sujet/salon à inspecter (optionnel, sinon tous confondus)")
async def avis_stats_cmd(interaction: discord.Interaction, salon: discord.TextChannel = None):
    gid = str(interaction.guild.id)
    tous_avis_guild = avis_data.get(gid, {})

    if salon:
        avis_liste = tous_avis_guild.get(str(salon.id), [])
        titre = f"📊 Avis pour #{salon.name}"
    else:
        avis_liste = [a for liste in tous_avis_guild.values() for a in liste]
        titre = "📊 Avis du serveur (tous salons confondus)"
